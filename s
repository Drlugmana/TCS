...
namespace DynatraceProblemUpdater.Services
{
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                _logger.LogInformation($"✅ Proceso finalizado siguiente actualizacion de datos en {TimeSpan.FromMilliseconds(_timeWaitLoop).TotalSeconds} segundos");
        ...
        private async Task LoadHistoricalDatabase(APIToken token, CancellationToken stoppingToken)
        {
            ...
            for(var startTime = startDate; startTime <= endDate; startTime = startTime.AddDays(daysToAdd))
            {
                ...
                try
                {
                    if(problems.Count > 0)
                    {
                        _logger.LogInformation("🟢 Insertando {Count} nuevos problemas del tenant {Tenant} en la BD...", problems.Count, token.Tenant);
                        await _problemRepository.InsertProblemsBatchAsync(problems);
                        _logger.LogInformation("✅ {Count} problemas del tenant {Tenant} insertados correctamente.", problems.Count,token.Tenant);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, $"❌ Error al procesar los problemas de Dynatrace del Tenant {token.Tenant} entre el {startTimeFilter} - {endTimeFilter}.");
                }
            }
        }
        ...
        private async Task<List<string>> FetchOpenProblemIdsAsync(APIToken token, CancellationToken stoppingToken)
        {
            ...
            do
            {
                ...
                {
                    _logger.LogError(ex, $"❌ Error en el Worker al obtener la lista de problemas de Dynatrace entorno {token.Environment} tenant {token.Tenant}.");
                }
         ...
         private async Task<List<DBProblem>> FetchProblemByIdsAsync(List<string> ids, APIToken token, CancellationToken stoppingToken)
 {
     ...
     foreach (var id in ids)
     {
         ...
         {
             _logger.LogError(ex, $"❌ Error en el Worker al obtener los datos del problema {id} de Dynatrace entorno {token.Environment} tenant {token.Tenant}.");
         }
     }
...
private async Task UpdateProblemDatabase(List<DBProblem> newProblems, List<DBProblem> updateProblems)
        {
            try
            {
                ...
                {
                    _logger.LogInformation("🟢 Insertando {Count} nuevos problemas en la BD...", newProblems.Count);
                    await _problemRepository.InsertProblemsBatchAsync(newProblems);
                    _logger.LogInformation("✅ {Count} problemas insertados correctamente.", newProblems.Count);
                }

                if(updateProblems.Count > 0)
                {
                    _logger.LogInformation("🟡 Actualizando {Count} problemas cerrados en la BD...", updateProblems.Count);
                    await _problemRepository.UpdateProblemsBatchAsync(updateProblems);
                    _logger.LogInformation("✅ {Count} problemas actualizados correctamente.", updateProblems.Count);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Error al procesar los problemas de Dynatrace.");
            }
        }
    }
}
